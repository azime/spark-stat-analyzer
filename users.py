...
# take newest date of new user
new_users_newest_name = new_users.reduceByKey(
    lambda a, b: a if a[1] > b[1] else b
)

# print('=========================')
# print(new_users_newest_name.count())
# print(new_users_newest_name.collect())
# print('=========================')


new_users_newest_name_collected = new_users_newest_name.collect()
new_users_oldest_date_collected = new_users_oldest_date.collect()

common.terminate(sc)

if len(new_users_newest_name_collected) != 0 and len(new_users_oldest_date_collected) != 0:
    # combine for insertion
    rows = []
    for (id_oldest, (name_oldest, timestamp_oldest)) in new_users_oldest_date_collected:
        for (id_newest, (name_newest, timestamp_newest)) in new_users_newest_name_collected:
            if id_oldest == id_newest:
                rows.append((id_oldest, name_newest, datetime.fromtimestamp(timestamp_oldest)))
